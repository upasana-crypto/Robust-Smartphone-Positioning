function [gnssEph,iSv] = ClosestGnssEph(allGpsEph,allGalileoEph,svIds,fctSeconds)
...
gnssEph = allGpsEph(1);%initialize gpsEph
numEph = 0;

%find all ephemeris corresponding to svIds(i)
for i=1:length(svIds)
    
    % check which satellite system: 1xx for gps, 6xx for galileo
    if svIds(i) > 599
        % for galileo satellites
        iThisSv = [allGalileoEph.PRN] + 600 == svIds(i);
        ephThisSv = allGalileoEph(iThisSv);
    else
        % for gps satellite
        iThisSv = [allGpsEph.PRN] + 100 == svIds(i);
        ephThisSv = allGpsEph(iThisSv);
    end
    
    if ~isempty(ephThisSv)
        ...
        if ageSeconds < (fitIntervalHours/2)*3600;
            numEph = numEph+1;
            gnssEph(numEph) = ephThisSv(iMin);
            gnssEph(numEph).PRN = svIds(i); % exchange sat id with sat id
            % coded with the satellite system (1xx: GPS, 6xx: Galileo)
...
if numEph==0
    gnssEph=[];iSv=[];%return empty matrices
end
